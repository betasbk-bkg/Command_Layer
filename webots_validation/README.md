...
- Webots R2025a
- Tested with Webots R2025a. Set the local executable with `--webots` or the `WEBOTS_EXECUTABLE` environment variable.
...
```powershell
python webots_validation\run_webots_validation.py --webots <path-to-webots.exe> --seeds 8 --timeout 7200
```

For a quick load test:

```powershell
python webots_validation\run_webots_validation.py --webots <path-to-webots.exe> --smoke --seeds 1 --timeout 600
```
