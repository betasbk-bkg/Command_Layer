...
from lambda2_utils import ascent_directions, kernel_sigma

LAMBDA2_SIGMA = kernel_sigma()
...
def lambda2_control_snapshot(
    positions: np.ndarray,
    alive: np.ndarray,
    policy: str,
) -> dict[str, object]:
    alive_pre = np.where(alive)[0]
    if len(alive_pre) <= 2:
        return {
            "lambda2": None,
            "eigengap": None,
    ...
    lam2, gap, dirs_all, degenerate = ascent_directions(positions[alive_pre], LAMBDA2_SIGMA)
    directions = None
    if policy == "s1_lambda2" and not degenerate:
        directions = {int(global_idx): dirs_all[local_idx] for local_idx, global_idx in enumerate(alive_pre)}
    return {
        "lambda2": float(lam2),
        "eigengap": float(gap),
...
def simulate_run(
    *,
    ...
    if policy not in {"baseline", "s1_lambda2"}:
    ...
    lambda2_ctrl_samples: list[float] = []
    lambda2_per_alive_samples: list[float] = []
    lambda2_post_samples: list[float] = []
    eigengap_samples: list[float] = []
    lambda2_low_population_steps = 0
    lambda2_degenerate_steps = 0
    ...
    for t in range(MAX_STEPS):
        ...
        lambda2_snapshot = lambda2_control_snapshot(positions, alive, policy)
        lam2_dirs = lambda2_snapshot["directions"]
        if lambda2_snapshot["low_population"]:
            lambda2_low_population_steps += 1
        else:
            lam2_value = float(lambda2_snapshot["lambda2"])
            n_alive_pre = int(lambda2_snapshot["n_alive"])
            lambda2_ctrl_samples.append(lam2_value)
            lambda2_per_alive_samples.append(lam2_value / max(1, n_alive_pre))
            eigengap_samples.append(float(lambda2_snapshot["eigengap"]))
            if lambda2_snapshot["degenerate"]:
                lambda2_degenerate_steps += 1
        ...
        if len(alive_indices) > 2:
            lam2_post, _, _, _ = ascent_directions(positions[alive_indices], LAMBDA2_SIGMA)
            lambda2_post_samples.append(float(lam2_post))
        for idx in alive_indices:
            ...
            drive = cohesion
            if isinstance(lam2_dirs, dict) and roles[idx] == "relay" and int(idx) in lam2_dirs:
                drive = lam2_dirs[int(idx)]
    ...
    result: dict[str, float | int | str] = {
        ...
        "fragmentation_mean": float(np.mean(fragmentation_values)) if fragmentation_values else 0.0,
        "max_fragmentation": float(np.max(fragmentation_values)) if fragmentation_values else 0.0,
        "lambda2_ctrl_mean": float(np.mean(lambda2_ctrl_samples)) if lambda2_ctrl_samples else 0.0,
        "lambda2_ctrl_min": float(np.min(lambda2_ctrl_samples)) if lambda2_ctrl_samples else 0.0,
        "lambda2_ctrl_final": float(lambda2_ctrl_samples[-1]) if lambda2_ctrl_samples else 0.0,
        "lambda2_post_mean": float(np.mean(lambda2_post_samples)) if lambda2_post_samples else 0.0,
        "lambda2_per_alive_mean": float(np.mean(lambda2_per_alive_samples)) if lambda2_per_alive_samples else 0.0,
        "lambda2_ctrl_n_steps": int(len(lambda2_ctrl_samples)),
        "eigengap_mean": float(np.mean(eigengap_samples)) if eigengap_samples else 0.0,
        "eigengap_min": float(np.min(eigengap_samples)) if eigengap_samples else 0.0,
        "eigengap_p01": float(np.quantile(eigengap_samples, 0.01)) if eigengap_samples else 0.0,
        "lambda2_low_population_steps": int(lambda2_low_population_steps),
        "lambda2_degenerate_steps": int(lambda2_degenerate_steps),
...
def make_design(seeds_per_condition: int, full: bool, study: str = "focused") -> list[dict[str, str | int]]:
    ...
    elif study == "s1_lambda2":
        for team in ["relay_sparse", "balanced_hetero", "relay_mid", "relay_rich"]:
            for map_mode in ["delayed", "scout_belief", "no_shared"]:
                for command_mode in ["autonomous", "crowd_vector", "consensus_gated", "stale_hold"]:
                    for stress_name in ["degraded", "severe"]:
                        condition_specs.append(
...
def simulate_from_spec(spec: dict[str, str | int]) -> dict[str, float | int | str]:
    return simulate_run(
        ...
        policy=str(spec.get("policy", "baseline")),
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--study",
        choices=["focused", "q2", "relay_sweep", "s1_lambda2"],
        default="focused",
