...
This computes the freshness-return ratio R = g*beta/c with bootstrap confidence
intervals and the embodiment-cost channel decomposition, using only the released
relay-sweep and Q2 delay-grid panels (no additional simulation). Expected values
are recorded in `reports/freshness_return_results.json`.
