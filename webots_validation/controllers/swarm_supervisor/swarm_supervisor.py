...
ROLE_COLORS = {
    "standard": [0.12, 0.36, 0.92],
    "scout": [0.06, 0.68, 0.28],
    "relay": [0.95, 0.70, 0.08],
    "payload": [0.86, 0.20, 0.16],
}

ROLE_LABELS = {
    "standard": ("STANDARD", 0x2C72FF),
    "scout": ("SCOUT", 0x14C85A),
    "relay": ("RELAY", 0xF2B21A),
    "payload": ("PAYLOAD", 0xE33A2E),
}
...
class BatchSupervisor(Supervisor):
    def __init__(self) -> None:
        ...
        self.body_color_fields = [node.getField("bodyColor") for node in self.nodes]
        self.config = self._load_config()
        self.output_path = Path(self.config["output_path"])
        self.output_path.parent.mkdir(parents=True, exist_ok=True)
        self.movie_path = os.environ.get("WEBOTS_SWARM_MOVIE_PATH")
        self.movie_match = self.config.get("movie_match")
    ...
    def _apply_role_colors(self, roles: list[str]) -> None:
        for idx, role in enumerate(roles):
            field = self.body_color_fields[idx]
            if field is not None:
                field.setSFColor(ROLE_COLORS[role])

    def positions(self) -> np.ndarray:
        return np.asarray([field.getSFVec3f()[:2] for field in self.translation_fields], dtype=float)

    def local_up_z(self, idx: int) -> float:
        axis_x, axis_y, axis_z, angle = self.rotation_fields[idx].getSFRotation()
        c = math.cos(float(angle))
        s = math.sin(float(angle))
        one_minus_c = 1.0 - c
        z_axis = np.array([float(axis_x), float(axis_y), float(axis_z)], dtype=float)
        norm = float(np.linalg.norm(z_axis))
        if norm <= 1e-9:
            return 1.0
        x, y, z = z_axis / norm
        return float(c + z * z * one_minus_c)

    def tipped_count(self) -> int:
        return sum(1 for idx in range(N_AGENTS) if self.local_up_z(idx) < 0.55)
    ...
    def run_trial(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        record_movie = self._should_record_movie(spec)
        self._apply_role_colors(roles)
        role_summary = " ".join(
            f"{ROLE_LABELS[role][0].lower()}={roles.count(role)}" for role in ("standard", "scout", "relay", "payload")
        )
        ...
        for step in range(MAX_STEPS):
            ...
            if record_movie and step % 8 == 0:
                payload_indices_live = [idx for idx, role in enumerate(roles) if role == "payload"]
                payload_progress_live = (
                    float(np.mean(delivered[payload_indices_live])) if payload_indices_live else float(np.mean(delivered))
                )
                self._set_movie_labels(
                    f"{spec['team']} | {spec['stress']} | {spec['command_mode']}\n"
                    f"{role_summary}\n"
                    f"step={step}  relay={relay_count}  delay={effective_delay}  alive={int(np.sum(alive))}/12  "
                    f"tipped={self.tipped_count()}  payload={payload_progress_live:.2f}"
                )
        ...
        if record_movie:
            self._set_movie_labels(
                f"{spec['team']} | {spec['stress']} | {spec['command_mode']}\n"
                f"{role_summary}\n"
                f"relay={relay_count}  delay={effective_delay}  alive={int(np.sum(alive))}/12  "
                f"tipped={self.tipped_count()}  safe={safe_delivery}  "
                f"attrition={attrition_rate:.3f}  score={operational_score:.1f}"
            )
            for _ in range(12):
                self.step(TIME_STEP)
            self._stop_movie()
    ...
    def _should_record_movie(self, spec: dict[str, Any]) -> bool:
        if not self.movie_path:
            return False
        if not self.movie_match:
            return True
        for key, value in self.movie_match.items():
            if str(spec.get(key)) != str(value):
                return False
        return True

    def _start_movie(self) -> None:
        movie_path = Path(str(self.movie_path))
        movie_path.parent.mkdir(parents=True, exist_ok=True)
        width = int(os.environ.get("WEBOTS_SWARM_MOVIE_WIDTH", "1280"))
        height = int(os.environ.get("WEBOTS_SWARM_MOVIE_HEIGHT", "720"))
        codec = int(os.environ.get("WEBOTS_SWARM_MOVIE_CODEC", "2"))
        quality = int(os.environ.get("WEBOTS_SWARM_MOVIE_QUALITY", "85"))
        acceleration = int(os.environ.get("WEBOTS_SWARM_MOVIE_ACCELERATION", "4"))
        caption = os.environ.get("WEBOTS_SWARM_MOVIE_CAPTION", "0") == "1"
        self.movieStartRecording(str(movie_path), width, height, codec, quality, acceleration, caption)
        self.movie_active = True
        print(f"WEBOTS_MOVIE_START {movie_path}", flush=True)

    def _set_movie_labels(self, text: str) -> None:
        try:
            self.setLabel(0, text, 0.02, 0.03, 0.040, 0x000000, 0.00, "Arial")
            self.setLabel(1, "ROLE LEGEND", 0.70, 0.03, 0.032, 0x000000, 0.00, "Arial")
            self.setLabel(2, "BLUE: STANDARD", 0.70, 0.072, 0.030, ROLE_LABELS["standard"][1], 0.00, "Arial")
            self.setLabel(3, "GREEN: SCOUT", 0.70, 0.108, 0.030, ROLE_LABELS["scout"][1], 0.00, "Arial")
            self.setLabel(4, "YELLOW: RELAY", 0.70, 0.144, 0.030, ROLE_LABELS["relay"][1], 0.00, "Arial")
            self.setLabel(5, "RED: PAYLOAD", 0.70, 0.180, 0.030, ROLE_LABELS["payload"][1], 0.00, "Arial")
            self.setLabel(6, "GOAL: GREEN BAND", 0.70, 0.222, 0.030, 0x008A2E, 0.00, "Arial")
        except Exception as exc:
            print(f"WEBOTS_MOVIE_LABEL_FAILED {exc}", flush=True)

    def _stop_movie(self) -> None:
        if not self.movie_active:
            return
        self.movieStopRecording()
        for _ in range(1000):
            if self.movieFailed():
                print("WEBOTS_MOVIE_FAILED", flush=True)
                self.movie_active = False
                return
            if self.movieIsReady():
                print(f"WEBOTS_MOVIE_READY {self.movie_path}", flush=True)
                self.movie_active = False
                return
            if self.step(TIME_STEP) == -1:
                break
        print("WEBOTS_MOVIE_NOT_READY_AFTER_WAIT", flush=True)
        self.movie_active = False
